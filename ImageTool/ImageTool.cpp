...
BOOL CImageToolApp::InitInstance()
{
	...
	if (cmdInfo.m_nShellCommand == CCommandLineInfo::FileNew)
		cmdInfo.m_nShellCommand = CCommandLineInfo::FileNothing; // 빈 창이 생성되지 않는다.
...
{
	theApp.m_pNewDib = &dib; // theApp라는 변수는 ImageTool.cpp파일에 선언된 CImageToolApp 클래스의 인스턴스 이름
	AfxGetMainWnd()->SendMessage(WM_COMMAND, ID_FILE_NEW); // 파일 새로 만들기 명령을 강제 발생, OnNewDocument 함수가 호출
}

// 출력창에 영상 처리 정보 문자열 출력
void AfxPrintInfo(CString message) // CString 타입 인자를 받는다.
{
	CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd(); // 메인 프레임의 주 창의 포인터를 받는다.
	pFrame->m_wndOutput.AddString(message); // 출력창에 문자열 추가
}

void AfxPrintInfo(LPCTSTR lpszFormat, ...) // 가변 인자를 받는다.
{
	CString message;
...
void CImageToolApp::OnEditPaste()
{
	...
	IppDib dib;
	if (dib.PasteFromClipboard()) // 클리보드에 영상이 있는 경우
		AfxNewBitmap(dib); // 새창으로 영상을 불러온다.
}
...
void CImageToolApp::OnUpdateEditPaste(CCmdUI *pCmdUI)
{
	...
	pCmdUI->Enable(IsClipboardFormatAvailable(CF_DIB)); // DIB데이터가 클립보드에 있을때만 복사를 허용한다, 활성화 시킨다.
}
